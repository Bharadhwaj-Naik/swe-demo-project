    ...
    return(
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50 flex items-center justify-center">
         <div className="text-center">
          <div className="inline-flex items-center justify-center w-16 h-16 mb-4 rounded-2xl bg-slate-100 mb-4">
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        <div className=" grid grid-cols-1 md:grid-cols-3 gap-6 mb-5">
          {stats.map((stat, index) => (
            <div key={index}
             className="grouup relative bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 p-6 hover:shadow-2xl hover:shadow-slate-300/50 transition-all duration-300 hover:-translate-y-1">
              <div className="flex items-center justify-between">
                ...
                <div className={`w-11 h-11 rounded-xl bg-linear to-br   ${stat.gradient}   shadow-lg  ${stat.shadowColor} flex items-center justify-center group-hover:scale-110 transition-transform duration-200}`}>
                  <stat.icon className="w-5 h-5 text-white" size={20} strokeWidth={2.5}/>
                </div>
                </div>
        ...
        {dashboardData.recentActivity && dashboardData.recentActivity.documents.length > 0 ||  dashboardData.recentActivity.quizzes.length > 0 ? (
          <div className="space-y-3">
            {[
              ...
              ...(dashboardData.recentActivity.quizzes || []).map(quiz=>({
                id: quiz._id,
                descreption:quiz.title,
                timestamp: quiz.lastAttempted,
                link:`/quizzes/${quiz._id}`,
                type:'Quiz'
              }))
            ].sort((a,b)=> new Date(b.timestamp) - new Date(a.timestamp)).map((activity, index)=>(
              <div key={activity.id || index} className="group flex items-center justify-between p-4 rounded-xl bg-slate-50/50 border border-slate-200/60 hover:bg-white hover:border-slate-300/80 hover:shadow-md transition-all duration-200">
                       ...
                       ? 'bg-gradient-to-r from-blue-400 to-cyan-500'
                         : 'bg-gradient-to-r from-emerald-400 to-teal-500'
                          ...
                          <p className="text-sm font-medium text-slate-900 truncate">
                            ...
                            <span className="text-slate-700">{activity.descreption}</span>
                          </p>
                  </div>
                ...
                {activity.link && (
                <a href={activity.link} className="ml-4 px-4 py-2 text-xs font-semibold text-emerald-600 hover:text-emerald-700 bg-emerald-50 rounded-lg transition-all duration-200 whitespace -nowrap">
                  View
                </a>
                )}
              </div>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-xs text-slate-500 mt-1">Start exploring documents and quizzes to see your activity here.</p>
          </div>
        )}
      </div>
    </div>
