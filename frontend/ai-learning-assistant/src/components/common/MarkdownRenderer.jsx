...
const MarkdownRenderer = ({ content }) => {
    return (
        <div className="text-nuetral-700">
        <ReactMarkdown 
          ...
          components={{
            ...
            blockquote:({node, ...props}) => <blockqoute className="border-l-4 border-neutral-300 pl-4 italic text-neutral-600 my-4"{...props}/>,
            code:({node,inline,className,children,...props})=>{
                const match = /language-(\w+)/.exec(className || '');
                return !inline && match ? (
                    <SyntaxHighlighter language={match[1]} style={darcula}   PreTag="div"{...props}>
                 ...
                 pre:({node,...props}) => <pre className="bg-neutral-100 p-2 text-white p-3 rounded md overflow-x-auto font-mono text-sm my-4"{...props}/>      
          }}
