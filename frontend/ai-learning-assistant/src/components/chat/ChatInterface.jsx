...
const ChatInterface = () => {
        ...
        const userMessage = { role: 'user', content: message, timestamp: new Date().toISOString() }; setHistory(prev => [...prev, userMessage]);
        setMessage('');
        setLoading(true);   
        try {
            const response = await aiService.sendMessage(documentId, message);
            const assistantMessage = { role: 'assistant', content: response.data.answer, timestamp: new Date(),relaventChunks:response.data.relaventChunks };
            setHistory(prev => [...prev, assistantMessage]);
        } catch (error) {
            console.error("Chat error:", error);
            const errorMessage = { role: 'assistant', content: "Sorry, something went wrong. Please try again later.", timestamp: new Date() };
    ...
    if(intialLoading){
        return( <div className="flex flex-col h-[70vh] bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl items-center justify-center shadow-slate-200/50">
            ...
            <div className="flex flex-col h-[70vh] bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 overflow-hidden">
                ...
                {history.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-center ">
                        ...
                        <h3 className="text-base font-semibold text-slate-900 mb-2">No messages yet. Start a conversation!</h3>
                ...
                {loading && (
                    ...
                    <div className="flex items-center gap-2 px-4 py-2 rounded-2xl rounded-bl-md bg-white border border-slate-200/60">
