...
import aisevice from "../../services/aiService";
...
const AIActions = () => {
    ...
    const handleGenerateSummary = async () => {
        ...
        try {
            const {summary}= await aisevice.generateSummary(documentId);
    ...
    const handleExplainConcept = async (e) => {
        ...
        try {
            const {explanation}= await aisevice.explainConcept(documentId,concept);
          ...
          <div className="bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 overflow-hidden">
            ...
            <div className="px-6 py-5 border-b border-slate-200/60 bg-linear-to-br from-slate-50/50 to-white/50">
               <div className="flex items-center gap-3">
                 <div className="w-10 h-10 rounded-xl bg-linear-to-br from-emerald-500 to-emerald-600 shadow-lg shadow-purple-500/25 ">
                    <Sparkles className="w-6 h-6 text-white" strokeWidth={2}/>
                    </div>
                    <div>
                    <h3 className="text-lg font-semibold text-slate-900">AI-Assitant</h3>
                    <p className="text-xs text-slate-500">Get insights, explanations and more from your document using AI</p>
                </div>
                     </div>
                       </div>
                <div className="p-6 space-y-6">
                      ...
                      <div className="flex-1">
                       <div className="flex items-center gap-2 mb-2">
                        <div className="w-8 h-8 rounded-lg bg-linear-to-br from-blue-100 to-cyan-100 flex items-center justify-center">
                            ...
                            <p className="text-sm text-slate-600 leading-relaxed">Get a concise summary of the document's content</p>
                            </div>
                            <button onClick={handleGenerateSummary} disabled={loadingAction === "summary"} className="shrink-0 h-10 px-5 bg-linear-to-r from-teal-500 to-teal-600 hover:from-teal-600 hover:to-teal-600 text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-blue-500/25 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95">
                                {loadingAction === "summary" ? (<span className="flex items-center gap-2">
                                    <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/>
                                    Generating...</span>) : ("summarize")}
                            </button>
                            </div>
                         </div>
                    {/* Explain Concept */}
                    <div className="group p-5 bg-linear-to-br from-slate-50/50 to-white rounded-xl border border-slate-200/60 hover:border-slate-300/60 hover:shadow-md transition-all duration-200">
                        <form onSubmit={handleExplainConcept}>
                            ...
                            <p className="text-sm text-slate-600 leading-relaxed mb-4">Enter a specific concept or term from the document that you want explained</p>
                            <div className="flex items-center gap-3">
                                <input
                                    type="text"
                                    value={concept}  
                                ...
                                <button type="submit" disabled={loadingAction === "explain" || !concept.trim()} className="shrink-0 h-11 px-5 bg-linear-to-r from-emerald-600 to-emerald-500 hover:from-emerald-600 hover:to-emerald-600 text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-purple-500/25 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95">
                                    {loadingAction === "explain" ? (<span className="flex items-center gap-2 ">
                                        <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/>
                                        Explaining...</span>) : ("Explain")}
                                </button>
                            </div>
                        </form>
                    </div>
