...
import {LayoutDashboard,Filetext,User,Logout,BookOpen,BrainCircuit,X} from 'lucide-react';
const Sidebar = ({ isSidebarOpen, toggleSidebar }) => {
    ...
    return <>
    <div className={'fixed inset-0 bg-back/30 z-40 md:hidden transition-opacity duration-300 ' + (isSidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none')} 
    onClick={toggleSidebar}
    aria-hidden="true"></div>
    <aside
        className={'fixed top-0 left-0 h-full w-64 bg-white/90 backdrop-blur-lg border-r border-slate-200/60 z-50 md:relartive md:w-64 md:shrink-0 md:flex md:flex-col md:translate-x-0 transition-transform duration-300 ease-in-out ' + 
        ...
        <nav className="flex-1 px-3 py-6 space-y-1.5">
            {navLinks.map((link) => (
                ...
                >
                    {({ isActive }) => (
                        <>
                            <link.icon size={18} strokeWidth={2.5} className={'transition-transform duration-200 ' + (isActive ? 'group-hover:scale-110' : 'text-slate-500')} />
                            {link.text}
                        </>
                    )}
                </NavLink>
